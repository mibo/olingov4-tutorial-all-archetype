...
public class DemoEntityProcessor implements EntityProcessor, MediaEntityProcessor {
  ...
  private void readEntityInternal(ODataRequest request, ODataResponse response, UriInfo uriInfo,
    ...
    if (responseEntity == null) {
      // this is the case for e.g. ${artifactId}.svc/Categories(4) or ${artifactId}.svc/Categories(3)/Products(999)
      throw new ODataApplicationException("Nothing found.", HttpStatusCode.NOT_FOUND.getStatusCode(), Locale.ROOT);
    }

    // 3. apply system query options

    // handle ${symbol_dollar}select
    SelectOption selectOption = uriInfo.getSelectOption();
    // in our example, we don't have performance issues, so we can rely upon the handling in the Olingo lib
    // nothing else to be done

    // handle ${symbol_dollar}expand
    ...
    // make sure that ${symbol_dollar}expand and ${symbol_dollar}select are considered by the serializer
   ...
   * Example request:
   * 
   * POST URL: http://localhost:8080/${artifactId}/${artifactId}.svc/Products
  ...
  public void deleteMediaEntity(ODataRequest request, ODataResponse response, UriInfo uriInfo)
     ...
     * DELETE request to /Advertisments(ID)/${symbol_dollar}value will be dispatched to the deleteMediaEntity(...) method
     * 
