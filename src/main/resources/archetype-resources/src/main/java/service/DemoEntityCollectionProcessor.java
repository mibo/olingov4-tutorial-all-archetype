...
public class DemoEntityCollectionProcessor implements EntityCollectionProcessor {
    ...
    if (segmentCount == 1) { // this is the case for: ${artifactId}/${artifactId}.svc/Categories
    ...
    } else if (segmentCount == 2) { // in case of navigation: ${artifactId}.svc/Categories(3)/Products
      ...
      if (lastSegment instanceof UriResourceNavigation) {
        ...
        // error handling for e.g. ${artifactId}.svc/Categories(99)/Products
    ...
    // we need the property names of the ${symbol_dollar}select, in order to build the context URL
  ...
  private List<Entity> applyTopQueryOption(List<Entity> entityList, TopOption topOption)
    ...
    if (topOption != null) {
      ...
      if (topNumber >= 0) {
        ...
        throw new ODataApplicationException("Invalid value for ${symbol_dollar}top", HttpStatusCode.BAD_REQUEST.getStatusCode(),
            Locale.ROOT);
      }
    }
  ...
  private List<Entity> applySkipQueryOption(List<Entity> entityList, SkipOption skipOption)
    ...
    if (skipOption != null) {
      ...
      if (skipNumber >= 0) {
        ...
        throw new ODataApplicationException("Invalid value for ${symbol_dollar}skip", HttpStatusCode.BAD_REQUEST.getStatusCode(),
            Locale.ROOT);
      }
    }
  ...
  private List<Entity> applyCountQueryOption(EntityCollection entityCollection, List<Entity> modifiedEntityList, 
    ...
    // handle ${symbol_dollar}count: always return the original number of entities, without considering ${symbol_dollar}top and ${symbol_dollar}skip
    if (countOption != null) {
